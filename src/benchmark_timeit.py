...
def run_profiling() -> None:
    ...
    for chunk_size in chunk_sizes:
        # Could use setup to also remove any existing zarr array, so not included in the final timing
        runner.timeit(
            name=f"write_to_zarr-{chunk_size}",
            stmt="benchmark_write_to_zarr(image, output_path, chunk_size)",
            setup=f"chunk_size={chunk_size}; output_path=Path('output/heart-example.zarr'); image = np.ones(shape=(500, 500, 500))",
            globals=globals(),
        )


if __name__ == "__main__":
    # To dump results to json, run with: python src/benchmark_timeit.py -o output/bench.json
